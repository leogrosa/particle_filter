...
// Same setup as mcl_bench.cpp, restricted to the GPU LUT method (GiantLUTCastGPU) only, so a
// Jetson rerun doesn't have to redo the bl/rm/cddt/pcddt/glt/rmgpu sweeps to add this method's
// numbers. Requires -DWITH_CUDA=ON. This is the "GPU version of the giant LUT" experiment: does
// GPU concurrency hide the LUT's DRAM latency once the O(1) gathers are actually parallelized?
// Directly comparable, column-for-column, to mcl_bench_rmgpu.cpp's CSV.
...
// Structurally identical to mcl_bench_rmgpu.cpp's bench_point, only the range method differs.
// GiantLUTCastGPU::numpy_calc_range_angles (a single GPU LUT-gather kernel launch) is used for
// ranges, then eval_sensor_model (RangeLib.h base RangeMethod class, CPU) for the weight
// accumulate -- the same GPU-ranges + CPU-sensor-model pairing RayMarchingGPU uses. gpu_range_ms
// times only the LUT-gather call, directly comparable to rmgpu's ms_gpu_range column.
//
// GiantLUTCastGPU::calc_range is batched-only (see RangeLib.h), so the synthetic observation is
// generated with a CPU RayMarching instance instead -- same approach as mcl_bench_rmgpu.cpp.
// Timing doesn't depend on the observed values, only on exercising the real GPU-gather +
// eval_sensor_model path. (Ranges are quantized LUT values; obs are exact RayMarching values.
// The absolute weights differ slightly from a fully self-consistent run, but the timed work --
// one GPU gather per (particle,angle) plus the CPU accumulate -- is exactly what we mean to measure.)
static PhaseTimes bench_point(GiantLUTCastGPU &range_method, RayMarching &obs_source, OMap &map, int max_particles,
    ...
    for (int iter = 0; iter < WARMUP_ITERS + TIMED_ITERS; ++iter) {
        ...
        // GiantLUTCastGPU::numpy_calc_range_angles does NOT auto-chunk internally (deliberately, to
        // avoid the ceil-based device-buffer overflow that RayMarchingGPU's version has). So the
        // caller must keep each call under CHUNK_SIZE. safe_batch uses integer division (a floor for
        // positive ints), so safe_batch*num_rays <= CHUNK_SIZE always -- the device output buffer
        // (sized CHUNK_SIZE) never overflows. Same working pattern as mcl_bench_rmgpu.cpp.
        ...
        // eval_sensor_model lives in the RangeMethod base class (RangeLib.h), inherited by
        // GiantLUTCastGPU unchanged -- CPU-only, uses map.world_scale and the sensor_model member
        // populated by the (also inherited) base set_sensor_model. Same GPU-ranges + CPU-sensor-model
        // pairing the library uses for RayMarchingGPU.
...
static void run_sweep(const char *method_name, GiantLUTCastGPU &range_method, RayMarching &obs_source, OMap &map,
...
int main(int argc, char **argv) {
    // Seed is a CLI arg (default 42), same pattern as mcl_bench_rmgpu.cpp -- lets a stats wrapper
    // run this binary under several seeds and average, separating real particle-count scaling from
    // seed-dependent workload variance (which map regions happened to get sampled).
    ...
    // GiantLUTCastGPU's own init builds the full flat LUT (~64s, one-time; see RangeLib.h) and
    // uploads it to the GPU -- paid here once, not inside the timed sweep.
    GiantLUTCastGPU lutgpu(map, max_range_px, THETA_DISCRETIZATION);
    lutgpu.set_sensor_model(sensor_table.data(), table_width);
    ...
    run_sweep("lutgpu", lutgpu, rm, map, sweep_60, angles_60, rng);
    run_sweep("lutgpu", lutgpu, rm, map, sweep_1080, angles_1080, rng);
